...
import { 
  ...
  X, Wallet, DollarSign, History as HistoryIcon, ShieldAlert, Lock, EyeOff, AlertCircle
} from 'lucide-react';
import { Doctor, Message, Invoice, MedicalEntry } from '../types';
...
const ClientPortal: React.FC<ClientPortalProps> = ({ doctor, onLogout }) => {
  ...
  const [viewingPetHistory, setViewingPetHistory] = useState<string | null>(null); // Pet Name
  ...
  // Datos simulados de historial para ejemplificar el bloqueo
  const mockHistory: MedicalEntry[] = [
    {
      id: 'm1',
      date: '21/06/2024',
      reason: 'Consulta Especialista',
      diagnosis: 'Insuficiencia Renal Leve - Requiere dieta especial y control mensual.',
      treatment: 'Fortekor 5mg (1 diaria), Dieta Renal Royal Canin.',
      attachments: [{ id: 'att1', name: 'Resultado_Laboratorio_Sanguineo.pdf', type: 'pdf', url: '#', date: '21/06' }]
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans relative">
      {/* BANNER DE DEUDA */}
      {hasDebt && (
        <div className="bg-rose-500 text-white px-8 py-3 flex justify-between items-center animate-in slide-in-from-top duration-500">
           <div className="flex items-center gap-3">
              <AlertCircle size={18} className="animate-pulse" />
              <p className="text-[10px] font-black uppercase tracking-widest">Atención: Tienes un saldo pendiente de ₡{pendingTotal.toLocaleString()}. Algunas funciones están limitadas.</p>
           </div>
           <button onClick={() => setActiveTab('payments')} className="text-[9px] font-black bg-white text-rose-500 px-4 py-1.5 rounded-full uppercase tracking-widest hover:bg-rose-50 transition-colors">Pagar Ahora</button>
        </div>
      )}

      {showNotificationToast && (
        <div className="fixed top-20 right-6 z-[100] animate-in slide-in-from-right-full">
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 px-8 py-4 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <div onClick={() => { setActiveTab('pets'); setViewingPetHistory(null); }} className="cursor-pointer">
        ...
        <div className="flex items-center gap-4">
           <button onClick={() => setActiveTab('payments')} className={`p-3.5 rounded-2xl transition-all relative ${activeTab === 'payments' ? 'bg-sky-500 text-white shadow-xl' : 'bg-slate-50 text-slate-400 hover:text-sky-500'}`}>
             <Wallet size={22}/>
             {hasDebt && <span className="absolute -top-1 -right-1 w-3 h-3 bg-rose-500 rounded-full border-2 border-white"></span>}
           </button>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-10">
        {activeTab === 'pets' && !viewingPetHistory && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6">
            <div className="bg-slate-900 rounded-[3rem] p-12 text-white shadow-2xl relative overflow-hidden">
               ...
               <div className="flex gap-6">
                  ...
                  <div onClick={() => setActiveTab('payments')} className={`${hasDebt ? 'bg-rose-500/20 border-rose-500/20' : 'bg-white/5 border-white/10'} px-10 py-6 rounded-3xl text-center border cursor-pointer hover:bg-rose-500/30 transition-all`}>
                    <p className={`text-4xl font-black ${hasDebt ? 'text-rose-500' : 'text-emerald-400'}`}>₡{pendingTotal.toLocaleString()}</p>
                    <p className="text-[10px] font-black uppercase text-slate-400">{hasDebt ? 'Saldo Pendiente' : 'Al Día'}</p>
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                ...
                { name: 'Max', species: 'Dog', breed: 'Labrador', status: hasDebt ? 'Pago Pendiente' : 'Al Día' }
              ].map(pet => (
                <div key={pet.name} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
                  ...
                  <button 
                    onClick={() => setViewingPetHistory(pet.name)}
                    className="w-full py-4 bg-slate-800 text-white font-black text-[10px] uppercase rounded-2xl hover:bg-sky-500 transition-all flex items-center justify-center gap-2"
                  >
                    {hasDebt ? <Lock size={14}/> : <HistoryIcon size={14}/>}
                    {hasDebt ? 'Historial Bloqueado' : 'Ver Historial Médico'}
                  </button>
                  {hasDebt && <p className="text-[8px] font-black text-rose-400 text-center uppercase tracking-widest mt-4">Liquida tu saldo para habilitar acceso</p>}
                </div>
              ))}
            </div>
          </div>
        ...
        {/* EXPEDIENTE DEL PACIENTE (CON BLOQUEO DE CONTENIDO) */}
        {activeTab === 'pets' && viewingPetHistory && (
           <div className="space-y-10 animate-in slide-in-from-right-10 duration-500">
              <div className="flex items-center gap-4 mb-6">
                 <button onClick={() => setViewingPetHistory(null)} className="p-3 bg-white rounded-2xl text-slate-400 hover:text-sky-500 shadow-sm"><ChevronRight className="rotate-180" size={24}/></button>
                 <h2 className="text-3xl font-black text-slate-800 tracking-tight">Expediente de {viewingPetHistory}</h2>
              </div>

              {mockHistory.map(entry => (
                <div key={entry.id} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
                   <div className="flex justify-between items-start mb-8">
                      <span className="text-xs font-black text-sky-600 tracking-widest uppercase">{entry.date}</span>
                      <span className="bg-slate-50 text-[10px] font-black px-4 py-1.5 rounded-full uppercase text-slate-400 tracking-widest">{entry.reason}</span>
                   </div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="space-y-6">
                         <div>
                            ...
                            <div className={`relative ${hasDebt ? 'blur-sm select-none' : ''}`}>
                               ...
                               {hasDebt && (
                                 <div className="absolute inset-0 flex items-center justify-center">
                                    <div className="bg-white/90 px-4 py-2 rounded-xl shadow-xl flex items-center gap-2">
                                       ...
                                       <span className="text-[10px] font-black text-slate-800 uppercase">Bloqueado por Deuda</span>
                                    </div>
                                 </div>
                               )}
                            </div>
                         ...
                         <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Tratamiento Recomendado</p>
                            <div className={`relative ${hasDebt ? 'blur-sm select-none' : ''}`}>
                               <p className="text-sm font-medium text-slate-600 italic leading-relaxed">{entry.treatment}</p>
                               {hasDebt && (
                                 <div className="absolute inset-0 flex items-center justify-center">
                                    <Lock size={20} className="text-rose-500 opacity-50"/>
                                 </div>
                               )}
                            </div>
                         </div>
                      ...
                      <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Resultados y Exámenes</p>
                         <div className="space-y-4">
                            {entry.attachments.map(att => (
                              <div key={att.id} className={`flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 ${hasDebt ? 'opacity-50' : ''}`}>
                                 <div className="flex items-center gap-3">
                                    <FileText size={18} className="text-sky-500"/>
                                    <span className="text-[10px] font-black text-slate-600 truncate max-w-[120px]">{att.name}</span>
                                 </div>
                                 {hasDebt ? (
                                   <button onClick={() => setActiveTab('payments')} className="p-2.5 text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><EyeOff size={16}/></button>
                                 ) : (
                                   <button className="p-2.5 text-sky-500 hover:bg-sky-50 rounded-xl transition-all"><Download size={16}/></button>
                                 )}
                              </div>
                            ))}
                            {hasDebt && (
                              <div className="pt-4 text-center">
                                 <p className="text-[9px] font-bold text-rose-400 uppercase tracking-widest leading-loose">Para descargar los resultados de laboratorio debes liquidar tu saldo pendiente.</p>
                              </div>
                            )}
                         </div>
                      </div>
             ...
             <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                   <div className="bg-emerald-50 p-8 rounded-[2rem] border border-emerald-100">
                      ...
                      <p className="text-3xl font-black text-emerald-700">₡{(clientInvoices.filter(i => i.paymentStatus === 'paid').reduce((acc, i) => acc + i.total, 0)).toLocaleString()}</p>
